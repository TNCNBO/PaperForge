...
Task: Handle paper according to input type and save to "./paperforge_lab/papers/id/id.md"
...
- Save path MUST be: ./paperforge_lab/papers/{PAPER_ID}/{PAPER_ID}.md

Processing Rules:
1. URL Input (input_type = "url"):
   - Use download_file_to tool with: url=<url>, destination="./paperforge_lab/papers/{PAPER_ID}/", filename="{PAPER_ID}.md"
   - Extract metadata (title, authors, year)
   - Return saved file path and metadata

2. File Input (input_type = "file"):
   - Use move_file_to tool with: source=<file_path>, destination="./paperforge_lab/papers/{PAPER_ID}/{PAPER_ID}.md"
...
Output Format (MANDATORY - EXACT FORMAT):
{
    ...
    "paper_path": "./paperforge_lab/papers/{PAPER_ID}/{PAPER_ID}.md (or null for text input)",
    "metadata": {
...
Example: If PAPER_ID=14, then paper_path should be "./paperforge_lab/papers/14/14.md"
"""
